...
class FaceDetector():
	...
	_feat_stride_fpn=None
	def __init__(self):
		print('[TVM FD] selection inference deivce.')
		self.ctx = tvm.gpu(0) 
		self.dtype = configs.tvm_fd_dtype
		self.prepare(configs.tvm_fd_rac, configs.tvm_fd_nms,None)
		print('[TVM FD] loading graph.')
		self.loaded_json = open(configs.tvm_fd_graph_json).read()
		print('[TVM FD] loading tvm lib.')
		self.loaded_lib = tvm.module.load(configs.tvm_fd_lib)
		print('[TVM FD] loading param.')
		self.loaded_params = bytearray(open(configs.tvm_fd_params, "rb").read())
		self.data_shape = configs.tvm_fd_data_shape
		print('[TVM FD] creation runtime graph.')
		self.module = graph_runtime.create(self.loaded_json, self.loaded_lib, self.ctx)
		print('[TVM FD] apply to param.')
		self.module.load_params(self.loaded_params)
		print('[TVM FD] created.')
		
	def __del__(self): 
		print('[TVM FD] destoryed.')

	def prepare(self, rac='net3', nms=0.4, fix_image_size=None ):
		self.rac = rac
		if fix_image_size is not None:
			self.data_shape = (1,3)+fix_image_size
		else:
			self.data_shape = (1,3)+self.default_image_size
	...
	def get_faces(self, img, threshold=0.5, scale=1.0):
		proposals_list = []
		scores_list = []
		landmarks_list = []
		'''
		...
		'''
		im = cv2.resize(img,(self.data_shape[2], self.data_shape[3]))
		...
		#input_face1 = tvm.nd.array(face_1.astype(self.dtype))
		#image1=np.transpose(im, (2, 0, 1))
		#image1=np.expand_dims(image1, axis=0)
		image1 = tvm.nd.array(im_tensor.astype(self.dtype))
		self.module.run(data=image1)
		#net_out = self.module.get_output(0)
		#print(type(net_out))
		...
		for _idx,s in enumerate(self._feat_stride_fpn):
			...
			if self.use_landmarks:
				...
				idx = _idx*2
			#scores = net_out[idx]
			scores = self.module.get_output(idx).asnumpy()#net_out[idx]
			scores = scores[:, self._num_anchors['stride%s'%s]:, :, :]
			#print(scores)
			idx+=1
			#bbox_deltas = net_out[idx]
			bbox_deltas = self.module.get_output(idx).asnumpy()
			#print(bbox_deltas)
			...
			scores = scores[order]
# 			print('inner score{}'.format(scores))
			proposals[:,0:4] /= scale
			...
			if self.use_landmarks:
				idx+=1
				landmark_deltas = self.module.get_output(idx).asnumpy()# net_out[idx].asnumpy()
