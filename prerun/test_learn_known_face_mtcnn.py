...
from utils.face_detector_mtcnn_tvm import FaceDetector as FD_MTCNN
import mxnet as mx
import tvm
_tpu=FD()
_tvm = FR()
...
for f in folders:
	if(not os.path.isdir(f)):
		...
		for _face in _faces:
			...
			tmp_img=cv2.resize(image[startY:endY,startX:endX], (164,266))
			print(tmp_img.shape)
			_align_face = _align.get_align_faces(tmp_img)
